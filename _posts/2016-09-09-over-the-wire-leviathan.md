...
Looking at the output of the code we can see that the program is using `strcmp` which is a C library function to compare two strings against one another. (Read more about `strcmp` [here]!(http://www.tutorialspoint.com/c_standard_library/c_function_strcmp.htm)) 
In this example, it's comparing the password input against the word "sex"; let's see if "sex" will work.
