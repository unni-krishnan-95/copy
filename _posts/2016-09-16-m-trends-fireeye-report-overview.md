...
* __Recovery & Backups__: Make sure you have a recovery and backup plan set up to constantly backup data and images. Also make sure that these backups are stored off-site and in locations that will be inaccessible to any malicious attacker. Your recovery plan should be implemented in your business continuity planning to make sure that in case of a breach or data loss, recovery can be put into action and in timely manner restore any data. Note, that this is critical for critical business data such as names, payments, etc.
* __Make sure any OSP is secure and reports any breaches__: Make sure that any OSP (Outsourced Service Provider) you are working is has good security in place, and reports any security breaches to you right away. This will allow you to monitor for any unusual activity or logins.
* __Monitor Internal Resources, Logins, and Privileged Accounts__: Constantly monitor failed login attempts on critical infrastructure devices, security tools, ACLs, etc. At the same time make sure you monitor any privileged account logins. Privileged Accounts can be used from separate user accounts to follow the “least privilege” principle – this will force admins to use the elevated account to make any critical changes, this helps decrease the attack surface an attacker might have if he cracks an admins account and not his Privilege Account, as it won’t have any permissions to make changes. Also monitoring of privileged account instead of user accounts makes it easier to pinpoint suspicious activity.
